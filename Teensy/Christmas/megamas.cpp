...
#define SPEED         10  // Speed of change
#define NUMBER        6  // Number of LEDs to change
#define TRANS_SPEED   1  // Transition speed
...
void Megamas::start() {
	for(int i = 0; i < NUM_LEDS; i++) {
		...
		target[i] = CHSV(0, 255, 200);
		current[i] = CHSV(0, 255, 200);
	}
	//FastLED.show();
}
...
void Megamas::tick() {
	...
	if(framenum > SPEED) {
		...
		for(int i = 0; i < NUMBER; i++) {
			switch(random(3)) {
				case 0:
					target[random(NUM_LEDS)] = CHSV(0, 255, 200);
					break;
				case 1:
					target[random(NUM_LEDS)] = CHSV(85, 255, 200);
					break;
				case 2:
					target[random(NUM_LEDS)] = CHSV(170, 255, 200);
					break;
			}
		}
	}
